...
- **Defaults:** model `gpt-5.6-sol`, effort `medium`, sandbox `read-only`, 10-minute execution timeout after queue admission. Set `effort: "low"` for mechanical work and `effort: "high"` for deep review or synthesis; raise the sandbox to `workspace-write` only for agents that must edit files.
